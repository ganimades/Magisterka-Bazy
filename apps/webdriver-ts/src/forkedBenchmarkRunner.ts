...
import { Builder, logging, promise, WebDriver } from 'selenium-webdriver'
import { benchmarks, BenchmarkType } from './benchmarks'
import { setUseShadowRoot } from './webdriverAccess'
import { lhConfig } from './lighthouseConfig';
import * as fs from 'fs';
import { BenchmarkError, BenchmarkOptions, config, FrameworkData, frameworks, JSONResult } from './common'
import * as R from 'ramda';
import { Benchmark, fileName, LighthouseData } from "./benchmarkClasses";
...
async function registerError(driver: WebDriver, framework: FrameworkData, benchmark: Benchmark, error: string): Promise<BenchmarkError> {
  ...
  try {
    ...
    return {imageFile: '', exception: error};
  }
}
